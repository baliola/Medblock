...
import {
  ...
  uamDetailHeader,
  uamDetailProfile,
} from "@/constants/contents/uam/detail";
...
} from "@/canister/declarations/patient_registry/patient_registry.did";
...
const IDCardInfo = ({ data }: { data: string }) => {
  ...
  return (
    <Flex align={"center"} gap={5}>
      <Icon
        as={uamDetailProfile.identity_card.icon}
        boxSize={8}
        color={"neutral.700"}
      />
      <Flex direction={"column"} color={"neutral.700"} gap={3}>
        ...
        <IDCardView isOpen={isIDCardOpen} onCLose={onIDCardClose} data={data} />
      </Flex>
    </Flex>
  );
};
...
const PatientDetail = ({
  ...
  getPatientList: () => Promise<PatientListAdminResponse | undefined>;
}) => {
  ...
  const { call: searchPatient, loading: loadingSearchPatient } =
    usePatientQuery({
  ...
  const { call: updateKycStatus, loading: loadingUpdateKycStatus } =
    usePatientQuery({
  ...
  const handleUpdateKycStatus = async (nik: string, kyc_status: KycStatus) => {
    try {
      ...
      searchPatient([{ nik, _type: [] }]);

      toast({
        title:
          Object.keys(kyc_status)[0].toLowerCase() === "approved"
            ? "KYC Approved"
            : "KYC Denied",
        description:
          Object.keys(kyc_status)[0].toLowerCase() === "approved"
            ? "Successfully aprrove KYC fot this user"
            : "Successfully deny KYC fot this user",
        status: "success",
      });
    } catch (error: unknown) {
    ...
    >
      ...
      <Flex direction={"column"} gap={8} flex={1}>
        ...
        <Flex direction={"column"} gap={5} ps={4} textTransform={"capitalize"}>
          ...
          />
          {Object.keys(patient.info.V1.kyc_status)[0].toLowerCase() !==
            "pending" && (
            <Flex align={"center"} gap={5}>
...
export default function UAMDetail({
  ...
  return <PatientDetail getPatientList={getPatientList} />;
}
