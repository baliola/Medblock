...
const MemberGroupList = (): ReactElement => {
  ...
  const [groupDetails, setGroupDetails] = useState<
    GetGroupDetailsResponse | null | undefined
  >(undefined);
  const [emrGroupInformation, setEmrGroupInformation] = useState<
    EmrListPatientResponse | null | undefined
  >(undefined);

  const { call: getGroupDetails, loading: loadingGetGroupDetails } =
    usePatientQuery({
  ...
  return (
    <>
      {groupDetails && profile ? (
        <>
          <GroupListBackNavigation
            props={{
              name: groupDetails.group_name,
            }}
          />
          <Flex maxHeight={"full"} pt={12}>
            <Flex h={"full"} overflowY={"auto"} w={"full"}>
              <Flex w={"full"} h={"full"} pb={"5.5rem"}>
                <Flex w={"full"} direction={"column"} rowGap={2} pb={4}>
                    ...
                    >
                      ...
                      >
                        <Flex direction={"column"} rowGap={1}>
                          ...
                          >
                            {member.name}{" "}
                            {groupDetails.leader_name === member.nik &&
                              "(Leader)"}{" "}
                            {profile.nik === member.nik && "(You)"}
                          </Text>
                          <Flex
                      ...
                      <GridItem my={"auto"} colSpan={1} display={"flex"}>
                        <DetailModal
                          props={{
        ...
        <Flex w={"full"} h={"full"}>
          <Flex h={"full"} w={"full"} margin={"auto"} justifyContent={"center"}>
            <Spinner
              size={"md"}
              colorScheme="primary"
              color="primary.700"
    ...
    <>
      ...
      >
        <Flex
          ...
          marginY={"auto"}
        >
          <Image src="/assets/female-doctor.png" alt="No Group" w={"50%"} />
...
export default function PatienMemberGroupList() {
  return (
    <PatientActor canisterId={patientCanisterId}>
      <Flex flex={1} w={"full"} direction={"column"}>
        <MemberGroupList />
      </Flex>
    </PatientActor>
  );
